...
<div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
    ...
    <div class="flex gap-2">
        <a href="{{ route('transactions.export.csv') }}" class="flex items-center space-x-2 bg-white border border-gray-300 text-gray-600 px-4 py-2 rounded hover:bg-gray-50 text-sm transition">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
            <span>CSV</span>
        </a>
        <button onclick="window.print()" class="flex items-center space-x-2 bg-white border border-gray-300 text-gray-600 px-4 py-2 rounded hover:bg-gray-50 text-sm transition">
...
<div class="bg-white p-4 rounded-lg shadow-sm mb-6">
    <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
        <div>
            ...
            <input type="date" name="start_date" value="{{ request('start_date') }}" class="w-full border rounded p-2 text-sm">
        </div>
        <div>
            <label class="text-xs font-bold text-gray-500 uppercase">Fim</label>
            <input type="date" name="end_date" value="{{ request('end_date') }}" class="w-full border rounded p-2 text-sm">
        </div>
        <div>
        ...
        <div>
            <button type="submit" class="w-full bg-dark text-white font-bold py-2 rounded hover:bg-gray-800 transition text-sm">Filtrar</button>
        </div>
    </form>
</div>
